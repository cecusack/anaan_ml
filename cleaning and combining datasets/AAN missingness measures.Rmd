...
dim(data1_noscales) 
round(sum(is.na(data1_noscales))/prod(dim(data1_noscales))*100, 2) # 0.61%
...
dim(data3_noscales) 
round(sum(is.na(data3_noscales))/prod(dim(data3_noscales))*100, 2) # 10.60%
...
dim(data4_noscales) 
round(sum(is.na(data4_noscales))/prod(dim(data4_noscales))*100, 2) # 9.17
...
# dx again
```{r}
old <- read_csv("~/Dropbox/Research/Coauthor/anaan_ml/cleaning and combining datasets/ANAANcombined_cec_2023-06-29.csv")
...
```{r warning=FALSE}
gg_miss_upset(dat)
gg_miss_var(dat[grepl("\\D$", colnames(dat))],
            facet = study)
# gg_miss_case(dat)
```
