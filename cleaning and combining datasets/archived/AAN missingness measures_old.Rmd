...
rm(list = ls.str(mode = 'character')) # clean up environment / remove all of Yuchen's character vectors
rm(data) # redundant with data7
```
...
data2 %>% select(pswq_1, pswq1r) %>% head()
# remove dashed
data2 <- data2[!grepl("(pswq_\\d{1,2})(r)", colnames(data2))] # so reverse coded items follow the pattern pswq1r
```

## bdi
```{r}
# data1[grepl("^bdi", colnames(data1))]
# data2[grepl("^bdi", colnames(data2))] # item 7 removed already
# data3[grepl("^bdi", colnames(data3))]
# data4[grepl("^bdi", colnames(data4))]
# data5[grepl("^bdi", colnames(data5))] # item 7 removed already
...
# X-squared = 9.479, df = 6, p-value = 0.1484
rcompanion::cramerV(df, ci=TRUE) 
# cramer's v = 0.1456, lower ci = 0.1003, upper.ci = 0.26
```
