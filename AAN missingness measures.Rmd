...
# dat combined
```{r}
dat <- plyr::rbind.fill(data1, data2, data3, data4, data5, data6, data7) # 461 x 386
dim(dat) # 460 x 327
...
## bdi
```{r}
#### dh3 - non clinical 20 item ####
# 15 and 17 already rescored
...
dat <- dat[!grepl("^fof_sc", colnames(dat))]
...
r15 missing pswq
```{r}
names(dat[grepl("^pswq", colnames(dat))]) # 11 items
